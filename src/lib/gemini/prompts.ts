...
// ────────────────────────────────────────────────────────────
// 자연어 드래프트 → 조항 배열 일괄 변환 프롬프트
// ────────────────────────────────────────────────────────────
export function buildBulkConvertPrompt(draft: string): string {
  return `투자자가 자신의 나쁜 투자 습관과 원칙들을 자유롭게 적은 드래프트입니다.
이것을 분석해서 독립적인 유언장 조항 배열로 변환하세요.
...
규칙:
- 드래프트에서 투자 원칙/습관에 해당하는 내용을 각각 독립 조항으로 추출
- 비슷한 내용은 하나로 합치되, 서로 다른 패턴은 분리
